...
export function AIAssistant({
  ...
  const MAX_WORDS = 30;
  const currentWordCount = inputValue.trim()
    ...
    const handleSuggest = async () => {
      ...
      try {
        ...
        const suggestionPrompt =
          "How can I improve my design? Make it structured in only three sentences. Give me specific issues and suggestions based on the current design context. Be concise and actionable.";
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (savedChats) {
        setMessages(
          JSON.parse(savedChats).map((msg: any) => ({
            ...msg,
  ...
  return (
    <Card className="relative overflow-hidden flex flex-col h-full border-t border-l-0 border-r-0 border-b-0 rounded-none bg-background text-foreground shadow-none">
      {showRatingPopup && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="bg-card border shadow-xl rounded-3xl p-6 w-full max-w-[260px] text-center relative animate-in fade-in zoom-in duration-200">
            ...
            {showRatingCongrats && (
              <p className="text-xs text-violet-500 font-medium animate-pulse mt-3">
        ...
        <CardTitle className="text-sm flex items-center justify-between w-full">
          <div className="flex items-center gap-3 ml-4">
            <div className="relative">
              ...
              <img
                ...
                alt="AI Mentor Profile"
                className="relative w-8 h-8 rounded-lg object-cover shadow-sm"
              />
            </div>
          ...
          {messages.length > 1 && (
            <Button
              variant="outline"
              size="sm"
              className="h-7 text-[10px] uppercase tracking-widest font-bold text-violet-600 dark:text-violet-400 border-violet-200 dark:border-violet-900/50 hover:bg-violet-50 dark:hover:bg-violet-900/30 rounded-full shadow-sm transition-all"
      ...
      <CardContent className="p-0 flex flex-col flex-1 min-h-0 bg-background relative">
        ...
        <div
          className="flex-1 overflow-y-auto pt-6 relative z-10"
          ...
          <div className="space-y-6 pb-6 px-6">
            {messages.map((message) => {
              ...
              return (
                <div
                  key={message.id}
                  className={`flex w-full animate-in fade-in slide-in-from-bottom-2 duration-300 ${isUser ? "justify-end" : "justify-start"}`}
                >
                  <div className="flex gap-3 max-w-[85%]">
                    {!isUser && (
                      <div className="flex-shrink-0 mt-auto mb-1">
                        <img
                          src="https://media.giphy.com/media/0JD7et5Wyv8m0mah8z/giphy.gif"
                          alt="AI Avatar"
                          className="w-8 h-8 rounded-full object-cover shadow-md border-0"
                        />
                      </div>
                    )}
                    ...
                    >
                      <div
                        className={`px-4 py-3 text-sm shadow-sm rounded-2xl overflow-hidden ${
                          isUser
                      ...
                      <div
                        className={`flex items-center gap-3 mt-1.5 px-1 ${isUser ? "flex-row-reverse" : "flex-row"}`}
                      >
                        ...
                        {!isUser && (
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className={`p-1.5 hover:bg-muted rounded-md transition-colors ${message.feedback === "up" ? "text-violet-500" : "text-muted-foreground hover:text-violet-500"}`}
                            >
                              <ThumbsUp
                                className={`w-3 h-3 ${message.feedback === "up" ? "fill-violet-500" : ""}`}
                              />
                            </button>
                            <button
                              onClick={() => rateMessage(message.id, "down")}
                              className={`p-1.5 hover:bg-muted rounded-md transition-colors ${message.feedback === "down" ? "text-red-500" : "text-muted-foreground hover:text-red-500"}`}
                            >
                              <ThumbsDown
                                className={`w-3 h-3 ${message.feedback === "down" ? "fill-red-500" : ""}`}
                              />
                            </button>
                          </div>
                        )}
        ...
        <div className="p-4 bg-card border-t z-20 relative">
          <div className="flex justify-between items-center mb-2 px-1">
            ...
            <span
              className={`text-[10px] font-bold ${isOverLimit ? "text-red-500 animate-pulse" : "text-muted-foreground"}`}
            >
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              size="icon"
              className="rounded-xl shadow-sm bg-violet-600 hover:bg-violet-700 text-white transition-all disabled:opacity-50 border-0"
            >
