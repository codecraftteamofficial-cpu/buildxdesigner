...
export function PreviewModal({
  ...
  const [localComponents, setLocalComponents] =
    useState<ComponentData[]>(components);
  const [localActivePageId, setLocalActivePageId] =
    useState<string>(activePageId);
  ...
  useEffect(() => {
    const handleScrollEvent = (event: Event) => {
      ...
      if (customEvent.detail?.elementId) {
        console.log(
          "Received scrollToElement event:",
          customEvent.detail.elementId,
        );
  ...
  const handleButtonClick = useCallback(
    (e: React.MouseEvent, component: any) => {
      ...
      if (component.type === "button" && component.props?.actions?.length) {
        const onClickActions = component.props.actions.filter(
          (a: any) => a.type === "onClick",
        );

        const executeSinglePreviewAction = async (
          action: any,
        ): Promise<void> => {
          if (!action) return;
          ...
          const executeHandler = async () => {
            try {
              ...
              if (action.handlerType === "scroll" && action.selector) {
                ...
                const root = contentRef.current || document;
                const success = await scrollToTarget(
                  action.selector,
                  2000,
                  root,
                );
              ...
              if (action.handlerType === "toggle" && action.selector) {
                ...
                if (elements.length === 0) {
                  ...
                  elements = previewContainer.querySelectorAll(
                    `[id="${cleanId}"], [data-component-id="${cleanId}"]`,
                  );
                  if (
                    elements.length === 0 &&
                    !selector.startsWith("#") &&
                ...
                if (elements.length > 0) {
                  elements.forEach((element) => {
                    ...
                    if (typeof action.toggleState === "boolean") {
                      ...
                      targetEl.style.display = action.toggleState
                        ? "block"
                        : "none";
                    } else {
                  ...
                  console.log(
                    `Toggled ${elements.length} element(s) with selector:`,
                    action.selector,
                  );
                  return true;
                } else {
              ...
              if (action.handlerType === "supabase" && action.supabaseTable) {
                ...
                Object.entries(dataMapping).forEach(([rawCol, valOrId]) => {
                  ...
                  const cleanId = valAsString.startsWith("#")
                    ? valAsString.substring(1)
                    : valAsString;
                  ...
                  if (allElements.length > 0) {
                    const previewElement = Array.from(allElements).find((el) =>
                      el.closest(".preview-container"),
                    );
                    if (previewElement) {
                  ...
                  if (element && "value" in element) {
                    ...
                    console.log(
                      `[Supabase Preview] Mapped "${col}" to input value: "${element.value}"`,
                    );
                  } else if (element) {
                    recordData[col] = element.innerText;
                    console.log(
                      `[Supabase Preview] Mapped "${col}" to text content: "${element.innerText}"`,
                    );
                  } else {
                    // Check if the static value is a JS expression like {currentUser.id}
                    if (
                      typeof valAsString === "string" &&
                      valAsString.startsWith("{") &&
                      valAsString.endsWith("}")
                    ) {
                      const expression = valAsString.substring(
                        1,
                      ...
                      try {
                        ...
                        console.log(
                          `[Supabase Preview] Evaluated expression for column "${col}":`,
                          recordData[col],
                        );
                      } catch (err) {
                        console.error(
                          `[Supabase Preview] Error evaluating expression "${expression}":`,
                          err,
                        );
                      ...
                      recordData[col] = valAsString;
                      console.log(
                        `[Supabase Preview] Mapped "${col}" to static/not-found value: "${valAsString}"`,
                      );
                    }
                  }
                });
                ...
                if (operation === "insert") {
                  ...
                  if (
                    !action.supabaseFilters ||
                    action.supabaseFilters.length === 0
                  ) {
                    if (recordData.id) {
                      ...
                      throw new Error(
                        'Update requires an "id" type field inside mapping data.',
                      );
                    }
                  } else {
                  ...
                  if (
                    ...
                    if (recordData.id) {
                      ...
                      throw new Error(
                        'Delete requires an "id" type field internally.',
                      );
                    }
                  } else {
                ...
                if (
                  ...
                  action.supabaseFilters.forEach((filter: any) => {
                    ...
                    const cleanId = filterValue.startsWith("#")
                    ...
                    if (allElements.length > 0) {
                      const previewElement = Array.from(allElements).find(
                        (el) => el.closest(".preview-container"),
                      );
                      if (previewElement) {
                    ...
                    switch (filter.operator) {
                      case "eq":
                        query = query.eq(filter.column, filterValue);
                        break;
                      case "neq":
                        query = query.neq(filter.column, filterValue);
                        break;
                      case "gt":
                        query = query.gt(filter.column, filterValue);
                        break;
                      case "gte":
                        query = query.gte(filter.column, filterValue);
                        break;
                      case "lt":
                        query = query.lt(filter.column, filterValue);
                        break;
                      case "lte":
                        query = query.lte(filter.column, filterValue);
                        break;
                      case "like":
                        query = query.like(filter.column, filterValue);
                        break;
                      case "ilike":
                        query = query.ilike(filter.column, filterValue);
                        break;
                      default:
                        query = query.eq(filter.column, filterValue);
                        break;
                ...
                console.log(
                  `[Supabase Preview] Executing ${operation} on "${table}"`,
                );
                ...
                if (result) {
                  ...
                  if (
                    ...
                    console.log(
                      "Dispatching supabase-data-update event for table:",
                      table,
                    );
                    window.dispatchEvent(
                      new CustomEvent("supabase-data-update", {
              ...
              if (
                (action.handlerType as string) === "showAlert" &&
                action.alertSelector
              ) {
                console.log(
                  "Executing showAlert action for:",
                  action.alertSelector,
                );
                ...
                window.dispatchEvent(
                  new CustomEvent("showAlertRequested", {
                    detail: { elementId },
                  }),
                );
                return true;
              ...
              if (action.handlerType === "condition" && action.conditionCode) {
               ...
               `,
                ...
                if (
                  isTrue &&
                  action.trueActionId &&
                  action.trueActionId !== "none"
                ) {
                  const trueAction = onClickActions.find(
                    (a: any) => a.id === action.trueActionId,
                  );
                  if (trueAction) await executeSinglePreviewAction(trueAction);
                } else if (
                  !isTrue &&
                  action.falseActionId &&
                  action.falseActionId !== "none"
                ) {
                  const falseAction = onClickActions.find(
                    (a: any) => a.id === action.falseActionId,
                  );
                  if (falseAction)
                    await executeSinglePreviewAction(falseAction);
                }
                return true;
              }
          ...
          if (success) {
            if (action.onSuccessUrl && action.onSuccessUrl !== "none") {
              console.log(
                "Chaining success navigation to:",
                action.onSuccessUrl,
              );
              handleNavigate(action.onSuccessUrl);
            } else if (
              action.onSuccessActionId &&
              action.onSuccessActionId !== "none"
            ) {
              const nextAction = onClickActions.find(
                (a: any) => a.id === action.onSuccessActionId,
              );
            ...
            if (action.onErrorUrl && action.onErrorUrl !== "none") {
              ...
              action.onErrorActionId &&
              action.onErrorActionId !== "none"
            ) {
              const errAction = onClickActions.find(
                (a: any) => a.id === action.onErrorActionId,
              );
        ...
        onClickActions.forEach((a: any) => {
          if (a.onSuccessActionId && a.onSuccessActionId !== "none")
            chainedActionIds.add(a.onSuccessActionId);
          if (a.onErrorActionId && a.onErrorActionId !== "none")
            chainedActionIds.add(a.onErrorActionId);
          if (a.trueActionId && a.trueActionId !== "none")
            chainedActionIds.add(a.trueActionId);
          if (a.falseActionId && a.falseActionId !== "none")
            chainedActionIds.add(a.falseActionId);
        });

        const rootActions = onClickActions.filter(
          (a: any) => !chainedActionIds.has(a.id),
        );
    ...
    const updateHeight = () => {
      ...
      if (maxBottom > 0) {
        setMeasuredHeight((prev) =>
          Math.abs(prev - maxBottom) > 1 ? maxBottom : prev,
        );
      }
    };
                      ...
                      <div
                        ...
                        style={{
                          ...
                          width:
                            ...
                            (typeof component.props?.width === "number"
                              ? `${component.props.width}px`
                              : component.props?.width) ||
                            "max-content",
                          height:
                            component.style?.height ||
                            (typeof component.props?.height === "number"
                              ? `${component.props.height}px`
