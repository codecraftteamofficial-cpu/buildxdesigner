...
export function ResizeHandle({
  ...
  return (
    <div
      ...
      {...props}
      style={{
        width: (autoSize && !isResizing) ? 'fit-content' : dimensions.width,
        height: (autoSize && !isResizing) ? 'fit-content' : dimensions.height,
        minWidth: `${minWidth}px`,
        minHeight: `${minHeight}px`
      }}
        onContextMenu={(e) => {
    // Don't stop propagation — let it bubble to canvas
    // But prevent it from triggering on resize handles themselves
